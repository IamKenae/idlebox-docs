...
- **最小纯 Rust 依赖** — 第三方代码仅限职责明确的 Rust crate；压缩使用 `flate2` + `miniz_oxide`，不启用 C zlib 后端
...
1. 尽量保持项目灵活、小巧、轻便、高性能，并守住单二进制与最小纯 Rust 依赖集的基础
...
归档 Applet 引入 `flate2` 与 `crc32fast`。`flate2` 会关闭默认特性并显式选择 `rust_backend`，将 DEFLATE 路由到 `miniz_oxide`，因此不会链接 zlib 或其他 C 压缩库。
